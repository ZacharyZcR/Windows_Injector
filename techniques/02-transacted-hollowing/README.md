...
### 360安全卫士 13.0.10.2001（开启核晶防护）

**测试环境：**
- OS: Windows 10 21H2 x64
- 360版本: 13.0.10.2001 正式版
- 防护级别: 核晶防护 + 行为监控全开
...
| 检测阶段 | 拦截情况 | 技术特征识别 |
|---------|----------|--------------|
| 事务创建 | ✅ 拦截 | `CreateTransaction` + `CreateFileTransacted` 组合触发 |
| 内存节映射 | ✅ 拦截 | `NtCreateSection(SEC_IMAGE)` 从事务文件创建检测 |
| 事务回滚 | ✅ 拦截 | `RollbackTransaction`后仍保留内存映射被识别 |
| 进程注入 | ✅ 拦截 | 基于无文件行为的进程创建拦截 |

**拦截提示:**
```
威胁名称: 无文件攻击 → 事务性代码注入
攻击技术: Transacted Hollowing (T1055.012变种)
风险等级: 极高
检测特征: NTFS事务 → 镜像节创建 → 事务回滚 → 进程创建
处置建议: 已隔离相关进程并清理残留映射
```

**技术对抗分析:**
事务性镂空虽然通过"事务回滚后文件不留痕迹"增强了隐蔽性，但360的行为分析引擎已经建立了完整的"事务生命周期"监控:

1. **CreateTransaction**调用频率异常
2. **SEC_IMAGE**节来源于事务文件
3. 事务主动回滚但内存映射持续存在
4. 进程从未经文件验证的节创建
...
**实战建议:**
此技术在2024年的环境下已属于"已知攻击模式",单纯的实现变体难以绕过现代EDR。如需使用,建议:
- 结合其他混淆技术(如API重定向、调用栈伪造)
- 增加前置的环境检测和规避
- 考虑混合多种注入技术
