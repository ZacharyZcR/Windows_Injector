...
| 技术环节 | 360拦截情况 | 分析 |
|---------|-------------|------|
| 挂起进程创建 | ✅ 拦截 | CREATE_SUSPENDED创建notepad.exe被标记为"潜在的进程注入前兆" |
| ntdll .data节修改 | ✅ 拦截 | 尝试修改g_ShimsEnabled时被360的内核页保护捕获 |
| .mrdata指针覆盖 | ✅ 拦截 | g_pfnSE_DllLoaded指针被360的"关键结构保护"规则拦截 |
| APC队列payload | ✅ 拦截 | 即使在上述操作后，NtQueueApcThread调用被拦截 |

**详细的拦截日志:**
```
【高危事件】内核数据劫持尝试
攻击类型: Early Cascade / Shim Engine Abuse
TTP编号: T1574.013 (Hijack Execution Flow)
威胁描述: 恶意代码尝试劫持ntdll.dll的内部回调机制
攻击行为链:
  1. 挂起进程创建(SUSPENDED) - 已告警
  2. 跨进程内存写入 → ntdll.dll数据段 - 已拦截
  3. 修改Shim Engine状态变量 - 已拦截
  4. APC队列载荷准备 - 已拦截
防护结果: 所有恶意操作均被阻止，目标进程已清理
```

**360检测技术分析:**

Early Cascade作为一项2024年新披露的技术,其被快速检测的原因:

1. **ntdll.dll保护机制**:
   - 360的"系统DLL保护"规则明确包含ntdll.dll
   - 任何跨进程写入ntdll内存的行为都被视为高危
   - 无论写入的是`.data`、`.mrdata`还是其他节

2. **Shim Engine回调验证**:
   - g_pfnSE_DllLoaded指针值必须满足特定条件
   - 360维护合法的回调函数地址白名单
   - 修改后的地址指向用户分配的内存,明显异常

3. **内存保护属性转换**:
   - payload内存从RWX到RX的转换被监控
   - 同时后续的`NtQueueApcThread`调用提供了第二重检测点

**技术评估与绕过可能性:**

这项技术的核心创新——利用Shim Engine回调——在360的"纵深防御"体系下效果有限:

- 0环(内核)保护覆盖了ntdll内存写入
- 3环(用户)行为监控捕获APC队列操作
- 启发式分析关联了整个行为链

绕过难度:**极高**。需要同时突破:
1. 内核驱动对ntdll的保护(EPT页表控制)
2. 用户态对APC队列的监控
3. 行为分析引擎的关联检测

**实际测试中的观察:**

即使关闭360的"实时防护",核晶模式的内核驱动仍然在底层生效。这说明了此技术的失败点在于内核层而非应用层。

**红队改进建议:**
1. 放弃对此技术的深入,转向完全无PEB/无ntdll修改的方案
2. 考虑基于硬件虚拟化的逃逸技术(如EPT Shadowing)
3. 或回归经典但可能有效的技术组合(Module Stomping + Doppelganging)
4. 最主要的:增加前置的环境检测,识别360核晶模式的存在

**结论**: 在360 13.0.10.2001核晶模式下,Early Cascade Injection的实战价值有限,主要作为技术研究用途
