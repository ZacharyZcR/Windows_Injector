...
| 检测点 | 拦截情况 | 技术细节 |
|---------|----------|----------|
| CreateProcess（挂起） | ✅ 拦截 | 360监控到`CREATE_SUSPENDED`标志，立即标记可疑进程创建 |
| NtUnmapViewOfSection | ✅ 拦截 | 核心API调用被360内核驱动捕获，识别为进程镂空特征 |
| 跨进程内存写入 | ✅ 拦截 | `WriteProcessMemory`到远程进程触发内存防护规则 |
| 进程恢复执行 | ✅ 拦截 | ResumeThread被拦截，防止完整攻击链完成 |

**拦截提示:**
```
威胁类型: 进程注入攻击 → 进程镂空 (T1055.012)
风险等级: 高危
检测行为: 创建挂起进程 + 卸载镜像 + 跨进程内存写入
操作建议: 已阻止并隔离相关进程
```

**分析结论:**
本技术作为经典的进程镂空实现，其完整的"创建-卸载-写入-恢复"行为链已被360全面收录。核晶模式下的内核监控能够捕获关键API序列，特别是`NtUnmapViewOfSection`的调用特征，使得此基础实现难以绕过。建议考虑使用无文件（Fileless）技术或API混淆方案。
